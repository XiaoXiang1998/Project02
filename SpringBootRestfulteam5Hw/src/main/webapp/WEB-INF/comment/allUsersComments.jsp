...
<body>
	...
	<div class="tab">
		...
		<button class="tablinks" onclick="openTab(event, 'commentsButton')">
			附上評論<span class="badge">${commentedPostsCount}</span>
		</button>
		<button class="tablinks" onclick="openTab(event, 'media')">
			附上照片/影片<span class="badge">${postsWithImagesCount}</span>
		</button>
	</div>
						...
						<div class="item">
							...
							<div class="info">
								...
								<div class="comment-container">
									...
									<c:if
										test="${loggedInMember.seller == true and not empty post.buyerrate}">
										<p>
											<a href="#" class="replyLink"
												data-target="replyFormContainer${post.commentid}"
												data-commentid="${post.commentid}">回覆買家</a>
										</p>
										<div id="replyFormContainer${post.commentid}"
											style="display: none;">
											<form action="/submitReply" method="post"
												class="bootstrap-frm" id="replyForm${post.commentid}">
												<span class="close-btn"
													onclick="closeReplyForm('${post.commentid}')">×</span>
												<h1 id="alter">
													回覆<span>請填寫回覆內容</span>
												</h1>
												<input type="hidden" name="memberId" value="${member.sid}">
												<input type="hidden" name="commentId"
													value="${post.commentid}"> <label><span>回覆內容:</span>
													<textarea id="replyContent${post.commentid}"
														name="replyContent" rows="10" cols="30" maxlength="100"
														placeholder="Your Reply" required></textarea></label> <span
													class="replyContent${post.commentid}" id="replyContent"
													data->輸入的字數:0/100</span><br />
												<div class="bit-com">
													評分:<span class="bit" id="bit1"></span> <span class="bit"
														id="bit2"></span> <span class="bit" id="bit3"></span> <span
														class="bit" id="bit4"></span> <span class="bit" id="bit5"></span>
												</div>
												<input type="hidden" name="rate"
													id="replyRate${post.commentid}" value="0"> <label><span>&nbsp;</span>
													<input type="submit" class="button" value="Send Reply" /></label>
											</form>
										</div>
									</c:if>
										...
										<c:forEach items="${seller.posts}" var="reply">
											<c:if test="${reply.repliedcommentid == post.commentid}">
												<div class="seller-reply">
													<p class="text">
														賣家回應<br /> <br />${reply.replayconetnt}</p>
													<c:if test="${not empty reply.replaytime}">
														<fmt:formatDate value="${reply.replaytime}"
															pattern="yyyy-MM-dd HH:mm" var="formattedReplayTime" />
	...
	<div id="commentsButton" class="tabcontent">
		<c:forEach items="${allMembers}" var="member">
			<div class="container">
				<c:forEach items="${member.posts}" var="post">
					<c:if
						test="${post.member.seller != true and not empty post.commentcontent and post.commentcontent ne ''}">
						<div class="item">
							<i class="avatar"></i>
							<div class="info">
								<p class="name">${member.name}</p>
