	...
	<div class="container-fluid py-5 mt-5">
		<div class="container py-5">
					...
					<div class="row g-4">
						<div class="col-lg-6">
							...
							<div class="your-class">
								<div>
									<img class="img-fluid w-100" src="${Good.titleImage}" alt="${Good.titleImage}" style="height: 400px;">
								</div>
								<c:forEach var="j" begin="0" end="${GoodImageNumber}" step="1" items="${GoodImage}" varStatus="loop">
									<div>
										<img class="img-fluid w-100" src="${j.imagePath}" alt="${j.imagePath}" style="height: 400px;">
									</div>
								</c:forEach>
								<c:forEach var="j" begin="0" end="${GoodFormatImagePathNumber}" step="1" items="${GoodFormatImagePath}" varStatus="loop">
									<div>
										<img class="img-fluid w-100" src="${j.goodImagePath}" alt="${j.goodImagePath}" style="height: 400px;">
									</div>
								</c:forEach>
							</div>
						...
						<div class="col-lg-6">
							...
							<!-- 商品種類 -->
							<h5 class="fw-bold mb-3" id="GoodPriceRange">${GoodBasicInfo.minprice}-${GoodBasicInfo.maxprice}$</h5>
							<!-- 商品價格(以範圍表示) -->
							<div class="row row-cols-2">
								<c:forEach var="j" begin="0" end="${GoodFormatNumber}" step="1" items="${GoodFormat}" varStatus="loop">
									<div class="col">
										<!-- 點擊規格表按鈕影響輪播圖片 -->
										<button class="btn btn-outline-primary round-pill CategoryItem w-100 mb-1">
											<img src="${j.goodImagePath}" alt="${j.goodImagePath}" style="width: 20px; height: 20px;">
							...
							<div class="input-group quantity mb-3 d-flex" id="BuyItemNumber" hidden>
								...
								<input type="text" class="form-control form-control-sm text-center border-0" value="1" id="BuyNumber" style="width: 10px;" hidden>
								<!-- 當點擊到庫存上限時 會出現紅字 -->
							...
							<div class="d-flex">
								<div class="col">
									...
									<form action="DirectlyBuy" id="orderDetail">
										<!-- 進入訂單環節(跳轉頁面) -->
										<!-- 下單後對應規格的庫存量會下降 -->
										<input type="text" class="form-control" id="orderformatimagepath" name="orderformatimagepath">
										<!-- 規格圖片 -->
										<input type="text" class="form-control" id="orderformatID" name="orderformatID">
										<!-- 規格表編號(OK) -->
										<input type="text" class="form-control" id="ordersellerID" name="ordersellerID" value="${sellerID}">
										<!-- 賣家編號 -->
										<input type="text" class="form-control" id="orderformatprice" name="orderformatprice">
										<!-- 對應規格單價 -->
										<input type="text" class="form-control" id="orderNumber" name="orderNumber">
										<!-- 購買數量 -->
										<input type="text" class="form-control" id="orderGoodName" name="orderGoodName" value="${Good.goodsID}">
										<!-- 商品名稱 -->
										<input type="text" class="form-control" id="orderGoodSize" name="orderGoodSize">
										<!-- 商品尺寸 -->
										<button type="submit" id="GoToOrder" class="btn btn-primary mt-3 mb-1">去結帳</button>
									</form>
								</div>
								<div class="col">
									<form action="" id="cartDetail">
										<!-- 進入購物車(透過ajax將值傳到後端) -->
										<input type="text" class="form-control" id="cartNumber" name="quantity">
										<!-- 購買數量 -->
										<input type="text" class="form-control" id="cartPrice" name="productPrice">
										<!-- 購買單價 -->
										<input type="text" class="form-control" id="cartformatID" name="ProductId">
										<!-- 規格表編號 -->
										<button type="button" id="GoToCart" class="btn btn-primary mt-3 mb-1">加入購物車</button>
									</form>
								</div>
			...
			<div class="col-lg-12">
				<nav>
					<div class="nav nav-tabs mb-3">
						<button class="nav-link active border-white border-bottom-0" type="button" role="tab" id="nav-about-tab" data-bs-toggle="tab" data-bs-target="#nav-about" aria-controls="nav-about" aria-selected="true">商品使用說明</button>
						<button class="nav-link border-white border-bottom-0" type="button" role="tab" id="nav-mission-tab" data-bs-toggle="tab" data-bs-target="#nav-mission" aria-controls="nav-mission" aria-selected="false">評論</button>
					</div>
				</nav>
					...
					<div class="tab-pane" id="nav-mission" role="tabpanel" aria-labelledby="nav-mission-tab">
						<c:if test="${not empty posts}">
							<div class="nav nav-tabs" id="nav-tab" role="tablist">
								...
								<button class="nav-link" id="star2-tab" data-bs-toggle="tab" data-bs-target="#star2" role="tab" aria-controls="star2" aria-selected="false">2顆星(${rateCounts[3]})</button>
								<button class="nav-link" id="star1-tab" data-bs-toggle="tab" data-bs-target="#star1" role="tab" aria-controls="star1" aria-selected="false">1顆星(${rateCounts[4]})</button>
								<button class="nav-link" id="comment-tab" data-bs-toggle="tab" data-bs-target="#comment" role="tab" aria-controls="comment" aria-selected="false">附上評論(${contentCount})</button>
								<button class="nav-link" id="photo-tab" data-bs-toggle="tab" data-bs-target="#photo" role="tab" aria-controls="photo" aria-selected="false">附上照片(${photosCount})</button>
								<!-- 添加缺少的标签 -->
							</div>
							<c:forEach var="post" items="${posts}">
								<div class="d-flex mb-4" id="comment-container">
									...
									<div class="ml-3">
										...
										<div class="d-flex align-items-center mb-2 time-and-details">
											<!-- 顯示時間和規格尺寸 -->
											...
											<!-- 規格尺寸 -->
											<p class="mb-0">規格: ${post.orders.formatgoodId.goodSize}</p>
										</div>
										...
										<c:if test="${not empty post.productphoto}">
											<div class="comment-photos">
												<img src="${post.productphoto}" class="img-fluid rounded mr-2" style="width: 100px; height: 100px;" alt="comment photo">
											</div>
										</c:if>
									</div>
								</div>
						...
						<form>
							<div class="d-flex align-items-center justify-content-start mb-3">
								<div class="rounded me-4" style="width: 100px; height: 100px;">
									<img src="${j.titleImage}" class="img-fluid rounded w-100 h-100" alt="" style="width: 150px; height: 250px;">
								</div>
								<input type="text" value="${j.goodsID}" name="GoodID" hidden>
								<div>
			...
			<button type="button" class="btn btn-primary">
				<h1 id="GoToSellerMarket">前往個人賣場</h1>
			</button>
	...
	<script>
		...
		$('.CategoryItem').click(
				function() {//取得對應規格編號[要顯示價格和數量]
					if ($('#BuyItemNumber').prop("hidden") == true) {//假若欄位被影藏
						...
						let targethidden = $('#BuyItemNumber').find(
								'div[class="input-group-btn"]');
						console.log(targethidden);
						targethidden.prop("hidden", false);
						let targethidden1 = $('#BuyItemNumber').find('input');
					...
					let formatID = $(this).find('span[class="formatID"]').prop(
							"innerHTML");//取得規格編號
					...
					let formatSize = $(this).find('span[class="CategorySize"]')
							.prop("innerHTML");
					//
		...
		$('#BuyNumber').change(
				function() {
					...
					let goodstock = $('#formatmsg').find(
							'span[id="remainingNumber"]').prop("innerHTML");
		...
		$('button[class="btn btn-sm btn-plus rounded-circle bg-light border"]')
				.click(
						function() {
							...
							if (InputNumber > Number(goodstock)) {
								$('#errormsg').prop("innerHTML", "操作不當");//最多只能購買剩餘的數量
								$(this).closest('div[id="BuyItemNumber"]')
										.find('input[id="BuyNumber"]').prop(
												"value", Number(goodstock));//不讓數字增加
							}
						})
		...
		$('#GoToOrder').click(function() {
			...
			if (buyerID == sellerID) {
				...
				let form = $(this).closest('form');
				// form.prop("method", "get"); form.prop("action", "inserttoshopcar.controller");
				// form.submit();
			}
				...
				.ready(
						function() {
							...
							function handleClick(event) {
								...
								loadComments(goodId, rate, content, photos,
										page);
							...
							function bindClickHandlers() {
								...
								$('#nav-tab').off('click').on('click',
										'.nav-link', handleClick);
							}
							...
							function handlePaginationClick(event) {
								...
								console.log("Current active tabId:",
										currentTabId);

								if (newPage !== page) { // 只有当点击的页码不等于当前页码时才加载新数据
									page = newPage;
									if (currentRate !== null) {
										loadComments(goodId, currentRate, null,
												null, page); // 加载对应星级的评论
									} else if (currentTabId === 'photo-tab') {
										loadComments(goodId, null, null, true,
												page); // 加载带有照片的评论
									} else if (currentTabId === 'comment-tab') {
										loadComments(goodId, null, true, null,
												page); // 加载带有评论的评论
									} else {
										loadComments(goodId, null, null, null,
												page); // 加载全部评论
									}
								}
							}
							...
							function bindPaginationClickHandlers() {
								console
										.log("Binding pagination click handlers");
								$('.pagination-link').off('click').on('click',
