				...
				<%@ include file="../../FrontDeskNav.jsp" %>
					<% HttpSession MemberSession=request.getSession(); MemberBean member=(MemberBean)
													...
													<div class="col">
														...
														<form action="DirectlyBuy" id="orderDetail">
															...
															<input type="text" class="form-control" id="orderGoodName"
																name="orderGoodName" value="${Good.goodsName}" hidden>
															<!-- 商品名稱 -->
											...
											<c:if test="${not empty posts}">
												<div class="nav nav-tabs" id="nav-tab" role="tablist">
													<button class="nav-link " id="all-tab" data-bs-toggle="tab"
														...
														aria-selected="true">全部(${txotalPostsCount})</button>
													...
													<div class="d-flex mb-4" id="comment-container">
														...
														<div class="ml-3">
															...
															<div
																...
																<!-- 規格尺寸 -->
																<p class="mb-0">規格: ${post.orders.formatgoodId.goodSize}
																</p>
															</div>
								...
								<div class="row">
									...
									<c:forEach var="j" begin="0" step="1" items="${relatedGood}">
										...
										<div class="col-3 card" style="height: 25rem;">
											<form>
												<img class="card-img-top" src="${j.titleImage}" alt="${j.titleImage}"
													style="height: 18rem;">
												...
												<div>
													...
													<div class="mb-2">
														<h5 class="fw-bold me-2">${j.minprice}-${j.maxprice}$
														</h5>
													</div>
												</div>
											</form>
						...
						<script>
							...
							//檢閱指定規格的數量和價格
							$('.CategoryItem').click(function () {//取得對應規格編號[要顯示價格和數量]
								if ($('#BuyItemNumber').prop("hidden") == true) {//假若欄位被影藏
									$('#BuyItemNumber').prop("hidden", false);//取消影藏
									let targethidden = $('#BuyItemNumber').find('div[class="input-group-btn"]');
									console.log(targethidden);
									targethidden.prop("hidden", false);
									let targethidden1 = $('#BuyItemNumber').find('input'); targethidden1.prop("hidden", false);
								}
								else {
									//假若從規格A按成規格B 數量直接回到1
								...
								let formatimagePath = "../../goodImages/" + formatimagePathTarget.substring(ps + 1); console.log(formatimagePath);
								//
							...
							$('#GoToOrder').click(function () {
								...
								if (buyerID == sellerID) {
									...
									if ($('#orderformatID').prop("value") == "") {
										...
										let form = $(this).closest('form');
										form.prop("method", "get"); form.prop("action", "DirectlyBuy");
										form.submit();
									}
							...
							$(document).on('click', 'div[class="col-3 card"]', function () {
								...
								let form = $(this).find("form"); console.log(form);
								form.prop("action", "goodDetail.controller"); form.prop("method", "get");
								form.submit();
							})
