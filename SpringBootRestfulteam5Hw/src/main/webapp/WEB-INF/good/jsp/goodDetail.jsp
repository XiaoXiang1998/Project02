					...
					<% HttpSession MemberSession=request.getSession(); MemberBean member=(MemberBean)
													...
													<div class="col">
														...
														<form action="" id="orderDetail">
															...
															<input type="text" class="form-control" id="ordersellerID"
																name="ordersellerID" value="${sellerID}">
															<!-- 賣家編號 -->
															<input type="text" class="form-control" id="orderformatprice"
																name="orderformatprice" >
															<!-- 對應規格單價 -->
						...
						<script>
							...
							$('#GoToSellerMarket').click(function () {
								...
								let form = document.createElement("form"); form.setAttribute("method", "get"); form.setAttribute("action", "sellerMarket");
								let input1 = document.createElement("input"); input1.setAttribute("type", "text"); input1.setAttribute("name", "SellerID"); input1.setAttribute("value", sellerID);
								form.append(input1);
							...
							$('button[class="btn btn-sm btn-plus rounded-circle bg-light border"]').click(function () {
								...
								if (InputNumber > Number(goodstock)) {
									$('#errormsg').prop("innerHTML", "操作不當");//最多只能購買剩餘的數量
									$(this).closest('div[id="BuyItemNumber"]').find('input[id="BuyNumber"]').prop("value", Number(goodstock));//不讓數字增加
								}
							})
							...
							$('button[class="btn btn-sm btn-minus rounded-circle bg-light border"]').click(function () {
								...
								if (InputNumber < 1) {
									$(this).closest('div[id="BuyItemNumber"]').find('input[id="BuyNumber"]').prop("value", 1);//最少購買一筆
								}
							})
							...
							$(document).on('click', 'div[class="d-flex align-items-center justify-content-start mb-3"]', function () {
